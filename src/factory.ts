...
/**
 * Create a logger with string-based log level and ensure output channel is ready
 ...
 * @returns Promise<Logger> - New Logger instance with output channel ready
 */
export async function createLoggerWithLevelAsync(
  name: string,
  level: string,
  outputChannel: boolean = true,
  context?: ExtensionContext | undefined
): Promise<Logger> {
  const logger = new Logger({ name, level, outputChannel, context });
  if (outputChannel) {
...
/**
 * Create a logger that automatically configures itself from VS Code settings with async output channel initialization
 * @param name - Logger name
 * @param configSection - The configuration section to read from
 * @param configKey - The configuration key for log level (default: 'logLevel')
 * @param defaultLevel - Default level if config is not found (default: 'info')
 ...
 * @returns Promise<Logger> - New Logger instance with output channel ready
 */
export async function createLoggerFromConfigAsync(
  name: string,
  configSection: string,
  configKey: string = 'logLevel',
  defaultLevel: string = 'info',
...
/**
 * Create a logger that monitors VS Code configuration changes automatically with async output channel initialization
 * This is a convenience function that always enables config monitoring
 * @param name - Logger name
 * @param configSection - The configuration section to read from
 * @param configKey - The configuration key for log level (default: 'logLevel')
 ...
 * @param outputChannel - Whether to use VS Code output channel (default: true)
 * @param context - Optional VS Code ExtensionContext (required for log contents access and automatic cleanup)
 * @returns Promise<Logger> - New Logger instance with automatic config monitoring enabled and output channel ready
 */
export async function createLoggerWithConfigMonitoringAsync(
  name: string,
  configSection: string,
  configKey: string = 'logLevel',
  defaultLevel: string = 'info',
  ...
  return createLoggerFromConfigAsync(
    name,
    configSection,
    configKey,
    defaultLevel,
